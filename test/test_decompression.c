...
void test_decompression_microschc(void) {
  ...
  const uint8_t context[] = {
      // Context
      0, 4, 0, 6, 0, 53,  // CHANGE THIS CONTEXT
  ...
  size_t packet_byte_len =
      decompress(packet, 150, DI_UP, schc_packet, schc_packet_byte_len, context,
                 context_len);
...
void test_several_packets(void) {
  ...
  memset(packet, 0, 150);
  packet_byte_len = decompress(packet, 150, DI_UP, schc_packet1,
                               schc_packet1_byte_len, context, context_len);

  assert(packet_byte_len == expected_packet1_byte_len);
  assert(memcmp(packet, expected_packet1, packet_byte_len) == 0);

  // Decompress expected_packet2
  ...
  const uint8_t schc_packet2[] = {0x00, 0x10, 0x80, 0x10, 0x8d, 0x59, 0x85,
                                  0xf5, 0x27, 0xc4, 0x00, 0xee, 0x2e, 0x81,
                                  0x64, 0x40, 0x84, 0x04, 0x12, 0x00, 0x00,
                                  0x00, 0x00, 0x00, 0x00};
  const size_t  schc_packet2_byte_len = sizeof(schc_packet2);

  memset(packet, 0, 150);
  packet_byte_len = decompress(packet, 150, DI_UP, schc_packet2,
                               schc_packet2_byte_len, context, context_len);
...
void test_on_byte_aligned_payload(void) {
  ...
  const uint8_t schc_packet1[] = {
      0x00, 0x0e, 0x80, 0x0e, 0x89, 0xf6, 0x88, 0x55, 0x48, 0xfc, 0x40,
      0x04, 0x79, 0xfb, 0x40, 0x43, 0x6c, 0xcc, 0xcc, 0xcc, 0xcc, 0xcd};
  const size_t schc_packet1_byte_len = sizeof(schc_packet1);
