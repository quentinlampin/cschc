...
void test_on_byte_aligned_payload(void) {
  uint8_t*     packet = (uint8_t*) pool_alloc(sizeof(uint8_t) * 100);
  const size_t packet_max_byte_len = sizeof(uint8_t) * 100;
  size_t       packet_byte_len;

  /**
   * @brief Perform SCHC decompression on schc_packet (DI = UP) using
   * short_context.
   *
   * @details The Rule Descriptor which matchs the packet is the 0 and the
   * schc_packet payload is byte aligned.
   */
  const uint8_t short_context[] = {
      ...
      0x13, 0xbe, 0, 2, 0, 1, 90, 2, 0x01, 0x6b, 0x01, 0x41,  // sid-coap-type
                                                              // bi/map/map
      0x13, 0xbc, 0, 4, 0, 1, 75 /*64*/, 1, 0x01, 0x6c,       // sid-coap-tkl
                                                              // bi/eq/ns
      ...
      0x00, 0x00, 0x00, 0x03,  // IPv6 Addr DST
      0x20, 0x01, 0x0d, 0xb8, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  ...
  const uint8_t schc_packet[] = {0x00, 0x0e, 0x80, 0x0e, 0x89, 0xf6, 0x88, 0x55,
                                 0x48, 0xfc, 0x40, 0x04, 0x79, 0xfb, 0x40, 0x43,
                                 0x6c, 0xcc, 0xcc, 0xcc, 0xcc, 0xcd};
  const size_t  schc_packet_byte_len = sizeof(schc_packet);
  ...
  const size_t expected_packet_byte_len = sizeof(expected_packet);

  packet_byte_len =
      decompress(packet, packet_max_byte_len, DI_UP, schc_packet,
                 schc_packet_byte_len, short_context, short_context_byte_len);

  assert(packet_byte_len == expected_packet_byte_len);
  assert(memcmp(packet, expected_packet, packet_byte_len) == 0);

  pool_dealloc(packet, packet_max_byte_len);
}

/* ********************************************************************** */

void test_coap_option_extended(void) {
  uint8_t*     packet = (uint8_t*) pool_alloc(sizeof(uint8_t) * 150);
  const size_t packet_max_byte_len = sizeof(uint8_t) * 150;
  size_t       packet_byte_len;

  /**
   * @brief Perform SCHC decompression on schc_packet1 (DI = UP) and
   * schc_packet2 (DI = UP) using short_context.
   *
   * @details The Rule Descriptor which matchs the packet is the 0 and the
  ...
  const size_t schc_packet1_byte_len = sizeof(schc_packet1);

  const uint8_t expected_packet1[] = {
      0x60, 0x0f, 0xf8, 0x5f, 0x00, 0x1d, 0x11, 0x40, 0x20, 0x01, 0x0d, 0xb8,
      0x00, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03,
      0x20, 0x01, 0x0d, 0xb8, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x20, 0x90, 0xa0, 0x16, 0x33, 0x00, 0x1d, 0x13, 0xed,
  ...
  const size_t expected_packet1_byte_len = sizeof(expected_packet1);

  packet_byte_len =
      decompress(packet, packet_max_byte_len, DI_UP, schc_packet1,
                 schc_packet1_byte_len, short_context, short_context_byte_len);

  assert(packet_byte_len == expected_packet1_byte_len);
  assert(memcmp(packet, expected_packet1, packet_byte_len) == 0);

  const uint8_t schc_packet2[] = {
  ...
  const size_t schc_packet2_byte_len = sizeof(schc_packet2);

  const uint8_t expected_packet2[] = {
      0x60, 0x0f, 0xf8, 0x5f, 0x00, 0x1d, 0x11, 0x40, 0x20, 0x01, 0x0d, 0xb8,
      0x00, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03,
      0x20, 0x01, 0x0d, 0xb8, 0x00, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x20, 0x90, 0xa0, 0x16, 0x33, 0x00, 0x1d, 0x13, 0xed,
  ...
  const size_t expected_packet2_byte_len = sizeof(expected_packet2);

  packet_byte_len =
      decompress(packet, packet_max_byte_len, DI_UP, schc_packet2,
                 schc_packet2_byte_len, short_context, short_context_byte_len);

  assert(packet_byte_len == expected_packet2_byte_len);
  assert(memcmp(packet, expected_packet2, packet_byte_len) == 0);
...
void test_rule_descriptor_0(const uint8_t* context,
  ...
  /**
   * @brief Perform SCHC decompression on packet (DI = UP) using context.
   *
  ...
  packet_byte_len = decompress(packet, packet_max_byte_len, DI_UP, schc_packet,
...
void test_rule_descriptor_4(const uint8_t* context,
  ...
  /**
   * @brief Perform SCHC decompression on packet (DI = DW) using context.
   *
  ...
  packet_byte_len = decompress(packet, packet_max_byte_len, DI_DW, schc_packet,
                               schc_packet_byte_len, context, context_byte_len);
